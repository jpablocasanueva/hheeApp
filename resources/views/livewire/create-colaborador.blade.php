<div>
    <h1 class="mb-4 px-4 mt-2 text-2xl text-slate-700 font-semibold">Agregar Nuevo Colaborador</h1>
    ...
    <form wire:submit="save" class="min-w-[30rem] flex flex-col gap-6 bg-white rounded-lg shadow p-6">
        <label class="flex flex-col gap-2">
            ...
            <input wire:model="form.cod_colaborador"
            @class([
                'px-3 py-2 rounded-lg',
                'border border-slate-300' => $errors->missing('form.cod_colaborador'),
                'border-2 border-red-500' => $errors->has('form.cod_colaborador'),
            ])
                placeholder="Codigo..."
                >
            @error('form.cod_colaborador')
        ...
        <label class="flex flex-col gap-2">
            ...
            <input wire:model.blur="form.nombre"
            @class([
                'px-3 py-2 rounded-lg',
        ...
        <label class="flex flex-col gap-2">
            <h3 class="font-medium text-slate-700 text-base">Local</h3>

            <select
            wire:model.blur="form.local_id"
            @class([
                'px-3 py-2 rounded-lg',
                'border border-slate-300' => $errors->missing('form.local_id'),
                'border-2 border-red-500' => $errors->has('form.local_id'),
            ])
            @error('form.local_id')
            <p class="text-sm text-red-500">{{ $message }}</p>
            @enderror

                <option value="" selected disabled>Seleccione Local</option>
